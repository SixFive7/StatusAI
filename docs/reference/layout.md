...
`pad = floor((avail - 4 - sum(cw)) / 8)`, the row can never exceed the budget:

```
row = 2 + sum(cw) + 8*pad  <=  2 + sum(cw) + (avail - 4 - sum(cw))  =  avail - 2
```
...
the bracket is charged at the wider of the two, which is the max across all rows. See
...
explicit `wNowBar` term, computed as a max across its column exactly as `wBar` is, so the constant
drops by ten and the total is unchanged. Both values are one higher than the measured truth (14,
was 24), which underfills the line by two columns and can never overflow it.
...
columns, so one long value (a `2d03h` reset, a three-digit projection) silently shortens every bar
on the line. That is why a design note for these rows is not finished until it states its character
cost.
...
`7d`, and have to agree to line up. The right column holds the scoped row alone, beside `7d`, and a
row beside another has nothing to line up with.
...
flagged instead, pending a review of the code; see
...
The account sits right of `5h`, followed by the product breakdown: each product's share of this
week's usage, in the account's text colour, with a dim `·` between them.
...
Four significant figures, a scaled unit, and always exactly six characters: `0,202k` `9,444k`
`240,7k` `1,397M` `46,92M` `112,0G`.
...
has nowhere to sit. Padding with a space doesn't help either, since it's invisible and
right-alignment was already inserting one.
...
invariant first and swaps the two separators, which keeps the ICU dependency out of the AOT binary.

Tool calls are plain counts with thousands separators, no unit.

## The grid

Nine columns, every cell built the same way, `[kind][scope] value`:
...
A scope keeps its column all the way across (🪵 in columns 1, 4, 7; 🌿 in 2, 5, 8; 🌳 in 3, 6, 9),
and the colour of the numbers backs that up, so a scope reads straight down without looking at the
glyphs.
...
Every row must open with a **single-width** glyph, because of where a terminal puts the ink of a
wide one.

The model line opens with a Nerd Font glyph and the metric rows with `5h` / `7d`, all single-width
and all at character column 1. A row opening with a double-width emoji also sits at character
column 1, but its *ink* is inset within a two-column advance, so it renders about half a cell right
of everything else.

That offset is fractional and the character grid is integral, so **no indent value can ever fix
it**. Two attempts showed as much: indent 1 put the emoji visibly right of the reference glyphs, and
indent 0 put it visibly left. Going from too far right to too far left with a one-column shift means
the target lies between two columns.

Leading each token row with `│`, a single-width box-drawing character, makes character column and
screen column the same number on every row, so the left edges coincide **by construction** rather
than by measurement. It also brackets the two token rows as one block, distinct from the metric rows
below.
...
These constraints survived several rounds of rejected alternatives.

Direction is relative to you, not to the model: 🔺 left, 🔻 came back. With the rows carrying send
and receive, an up-arrow on the sent row is the only reading that doesn't fight the layout.

💾 and 📖 carry no arrow. An earlier design encoded `[direction][scope][cache]` in three icons,
which put a down-arrow on both. That was correct, since cache writes and cache reads are both
*input* tokens, but it invites reading the arrow against the cache instead of against the
conversation, two reference frames in one cell. The arrow was also redundant there: it only ever
distinguished fresh input from output, and never varied in the cache columns.

The orientation has to be the whole glyph, not a detail inside it. 📥/📤 failed because the
direction lived in a ~3-pixel arrow inside two otherwise identical trays, so 95% of the glyph was
noise. Solid triangles work because the orientation *is* the glyph.

No coloured up/down emoji pair has different hues, so the triangle sets are orientation-only by
necessity. Every icon is default-emoji-presentation, with no variation selectors, so width and
colour are guaranteed by the codepoint alone.
...
`#0C0C0C` (the Claude Code profile sets no scheme) under a wallpaper at 10% opacity, which can lift
it to `#242424` where the image is white; the docs' pane ground is `#16161E`. Contrast is the WCAG
ratio against each.

| colour | where | Campbell | lightest wallpaper | `#16161E` |
|---|---|---|---|---|
| `#7DCFFF` cyan | bar cells 1 to 7, percentages under 70; bold, the token-grid totals | 11,40 | 9,05 | 10,48 |
| `#E0AF68` amber | bar cells 8 and 9, percentages 70 to 89, `warning` labels, the meters notice | 9,78 | 7,76 | 8,99 |
| `#F7768E` red | bold: bar cells 10+ and `✗`, percentages 90+, `critical` labels, `→` when 100% lands before the reset, projections past 100% (in a projection only while the row is under 100%), and the `⚠` rows except the meters notice; plain: removed lines | 7,39 | 5,87 | 6,80 |
...
Forest is the only one chosen against these grounds. It is X11 `forestgreen` `#228B22` (hue 120
degrees, saturation 61%) lifted from 34% to 40% lightness, because `#228B22` itself is 4,10:1 on the
...
CIELAB chroma 76 against the 41 of `↻`'s `#A6E3A1`, and a CIEDE2000 difference of 21,7 from it and
27,3 from the reset time's `#C6F6C1`. The screenshot this was checked against draws the ground at
`#1B1B1B` on the median and `#222222` at the 99th percentile, between the table's first two
columns.
...
a bare `/` and no spaces. It measured 130 of 137 columns with the largest realistic figures, and the
euro rate was the last field that could grow it.

The euro figure is a conversion of Anthropic's **client-side list-price estimate**. On a
subscription plan nobody is billed that amount; it only gauges relative effort.
...
height as a split one and reads far worse. A source that reported a real *zero* says nothing. The
row exists so that a blank figure can always be told apart from a real one.
...
[limits.md](limits.md#on-credit). It has the same red and the same budget, and is cut the same way
as any other reason if the terminal is too narrow for it.
...
Until the first API response of a context (in every new session, and again after `/clear`), Claude
...
together or not at all. A payload in exactly that shape is a source with nothing to report yet, like a
`cost` of 0: no response has been measured, so cship's `0%` is the right reading, and the row says
nothing.
...
row until its first response. Its fixture for a real fresh session was `used_percentage: 0`,
which Claude Code never sends, and cship draws `null`, `0` and an absent field as the same
`○○○ 0%`, so comparing the visible text could not tell them apart. A fixture for this path has to
be what the payload builder really emits, not a plausible stand-in.
...
2.1.234 and 2.1.275 to 2.1.280 alike. A session left open has no file for as long as nobody types,
...
segment shows at that point. On its own it was too short, though. A session left idle past it went
...
to and nowhere to insert, and skipping the insert, which is what this used to do, printed **nothing
at all**. An empty status line is the one output indistinguishable from the binary being
uninstalled, crashed, or never run, and it arrived at the worst possible moment: cship parses the
same stdin we do, so a malformed payload silences both of us at once and takes the diagnostic with
it.

The block is now emitted on its own instead. Almost nothing in it comes from stdin (the limit rows
are the registry cache and the API, the account line is the credentials file), so what is really
lost is the two token rows and the four meta figures, each of which already degrades to a sentinel
with a stated reason.

This has two consequences for the layout:

- The meta segment has no host line to ride on, so it takes the first row of the block and opens
  with `—`, the sentinel for a source that reported nothing at all, which here is cship. It is
  single-width, so this row's left edge lands on column 1 exactly as `│` and `5h` do, by
  construction and not by measurement. If the segment is empty because cost and duration both
  really read zero, the row is not drawn at all.
- The width budget is unchanged. The four columns of host padding are Claude Code's, not cship's,
  and the indent is still ours. Indent, marker and space cost 3 columns, which puts the meta
  segment's measured worst case at 133 of 137, the same width the token grid is built to.

cship being *absent* is a different case, and takes a different path. `RunCship`'s `catch` returns
the raw stdin, which is a non-empty line, so the block is inserted beneath the echoed JSON exactly
as usual. Standalone happens only when cship ran and said nothing.
